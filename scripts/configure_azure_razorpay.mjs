...
try {
  ...
  console.error("Standard containerapp update error, trying ARM REST for template update...", updateErr.message);

  if (subscriptionId) {
    // Fallback: ARM REST patch for container template (image + env vars)
    ...
    if (container) {
      ...
      // Update or add RAZORPAY env vars
