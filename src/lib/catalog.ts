...
export async function createCatalogItem(item: CatalogItemInsert): Promise<{ data: CatalogItem | null; error: Error | null }> {
  ...
  try {
    ...
    const { data, error } = await withTimeout(query, 120000, "Catalog save timed out. Please check your network connection.");
...
export async function updateCatalogItem(
  ...
  try {
    ...
    const { data, error } = await withTimeout(query, 120000, "Catalog update timed out. Please check your network connection.");
